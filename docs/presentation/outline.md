...
> 자료 출처: PRD·spec·plan·working-memory + `no-home-backend/docs` (AI 챗봇 설계·구현·트러블슈팅)
>
> 범례: `⭐` 강조 슬라이드 / `<!-- TODO -->` 팀이 직접 채울 항목(일정·사진·회고·시장조사)
...
- **시각자료**: 원본 XML vs 우리 서비스 결과 카드 비교
...
### 2-1. 전체 마일스톤 타임라인
- **메시지**: 문서 기반 Sprint로 단계적 추진
- 마일스톤 로드맵 (plan.md 근거):
  - M0 계획·하네스 / M1 골격·DB / M2 실거래가 데이터(Sprint2~5) / M3 회원·인증(Sprint6,8) / M4 화면·지도 연결(Sprint7~9) / M5 추가기능 / M7 제출
  - Sprint 0 → 12까지 진행 (법정동 드롭다운 안정화까지)
  - AI 챗봇은 별도 트랙(Phase1~3, 06-21~23)
- **시각자료**: 마일스톤 × Sprint 간트/타임라인

### 2-2. 개인별 역할 & 일정
- **메시지**: 역할 분담과 담당 영역
- <!-- TODO: 팀원 실명·개인별 담당 Sprint·기간을 채울 것. (git 브랜치 단서: feature/jeongheon-chatbot=AI 챗봇, feature/minsik-folder-path-update=인프라/폴더구조; repechage-team 조직) -->
- 역할 축(예시 틀): Backend(검색·데이터·인증) / Frontend(화면·지도·상태) / AI(챗봇·Tool Calling)
- **시각자료**: 담당자별 스윔레인 타임라인
...
### 3-1. 경쟁 서비스 비교
- **메시지**: 기존 부동산 서비스 대비 위치
- <!-- TODO: 실제 비교 대상 확정 (예: 호갱노노, 직방, 네이버 부동산, KB부동산, 아실). 비교 항목별 O/X 표 채울 것 -->
- 비교 축(틀): 실거래가 제공 / 지도 / 공공데이터 직접 연동 / **자연어 AI 검색** / 무료·진입장벽
- 차별 포인트: 대부분 서비스가 검색 UI는 제공하나 **자연어로 필터·화면을 조작하는 AI 에이전트는 희소**
- **시각자료**: 경쟁사 비교 매트릭스
...
- 요구사항 매핑표 (F701/F702/F712~716/F720/지도) + 구현 근거
- **시각자료**: 체크리스트 표
...
- **시각자료**: 아키텍처 다이어그램 (architecture-diagram)
...
### 6-1. 화면 흐름도
- **메시지**: 사용자 동선 한눈에
- 진입 → (검색: 거래유형→지역→가격→검색) → 결과+지도 → 항목선택/지도포커스 → AI 조작 → 회원 로그인/내정보
- **시각자료**: 화면 흐름 플로우차트

### 6-2. 라이브 시연 (DEMO) ⭐
- 시연 4단계 (상세: `demo-script.md`)
  1. 지역+거래유형 검색 → 결과+지도
  2. 가격 필터 조정 → 재검색

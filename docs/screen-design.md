...
| SCR-03 | 회원 정보 화면 | [memberInfo.png](screens/memberInfo.png) | 로그인한 사용자의 계정 정보 조회, 수정/로그아웃/탈퇴 진입 |
| SCR-04 | 회원 삭제 후 로그인 화면 | [accountDelete.png](screens/accountDelete.png) | 회원 탈퇴 완료 메시지와 재로그인 상태 표시 |
| SCR-05 | 공지사항 관리 화면 | [notice.png](screens/notice.png) | 등록된 공지 목록 조회, 관리자 공지 등록/수정/삭제 |
| SCR-06 | 관리자 회원 검색 화면 | [memberSearch.png](screens/memberSearch.png) | 관리자가 기존 회원을 검색하고 기본 정보를 확인 |
| SCR-07 | 메인 검색 초기 화면 | [mainScreen.png](screens/mainScreen.png) | 검색 조건 입력, 지도 표시, 검색 전 상태 안내 |
| SCR-08 | 강남구 검색 결과 화면 | [gangnamguSearch.png](screens/gangnamguSearch.png) | 지역/거래유형/가격 조건 기반 결과 목록 및 지도 마커 표시 |
| SCR-09 | 매매 높은 가격순 검색 화면 | [thirdHighPrice.png](screens/thirdHighPrice.png) | 매매 실거래가 가격 범위와 높은 가격순 정렬 결과 확인 |
...
![회원 정보 화면](screens/memberInfo.png)
...
![회원 삭제 후 로그인 화면](screens/accountDelete.png)
...
![메인 검색 초기 화면](screens/mainScreen.png)
...
![강남구 검색 결과 화면](screens/gangnamguSearch.png)
...
### SCR-09. 매매 높은 가격순 검색 화면

![매매 높은 가격순 검색 화면](screens/thirdHighPrice.png)

| 항목 | 내용 |
| --- | --- |
| 목적 | 매매 거래에서 실거래가 범위와 높은 가격순 정렬 조건을 적용한 결과를 확인한다. |
| 주요 UI | 매매 탭, 실거래가 구간 슬라이더/입력값, 높은 가격순 정렬, 검색 결과 카드, 지도 마커 |
| 주요 동작 | 최소/최대 실거래가를 조정하고 높은 가격순으로 정렬하여 고가 매물부터 확인한다. |
| 검증 포인트 | 거래 유형이 매매일 때 보증금/월세 필터가 아닌 실거래가 필터가 표시되어야 한다. |
